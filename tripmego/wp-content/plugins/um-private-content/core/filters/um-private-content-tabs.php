...
function um_private_content_add_tab( $tabs ) {
    ...
    $tab_title = um_get_option('tab_private_content_title');
    $tab_title = ! empty( $tab_title ) ? $tab_title : __( 'Private Content', 'um-private-content' );

    $tab_icon = um_get_option('tab_private_content_icon');
...
function um_private_content_user_add_tab( $tabs ) {
    if ( ! um_is_myprofile() || ! um_get_option( 'show_private_content_on_profile' ) ) {
