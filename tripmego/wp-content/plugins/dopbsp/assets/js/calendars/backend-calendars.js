...
var DOPBSPBackEndCalendars = new function(){
     ...
     */
    this.init = function(){
        $('.dopbsp-calendar-item').unbind('click');
        $('.dopbsp-calendar-item').bind('click', function(){
            var id = $(this).attr('id').split('DOPBSP-calendar-ID-')[1],
                user_id = parseInt($(this).attr('data-user_id'));  
            DOPBSPBackEndCalendar.init(id, user_id);
        });
    };
